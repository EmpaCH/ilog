...
import { useDownloadAfsEntry } from '../../apis/dataset/useDownloadAfsEntry'
import { useGetAfsEntries } from '../../apis/dataset/useGetAfsEntries'
import { useUploadAfsDataSet } from '../../apis/dataset/useUploadAfsDataSet'
import { useUploadAfsDummyFile } from '../../apis/dataset/useUploadAfsDummyFile'
...
export function DatasetUploadAfsPage() {
  ...
  const dummyMutation = useUploadAfsDummyFile()
  ...
  const dummyError = dummyMutation.isError ? toErrorInfo(dummyMutation.error) : null
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-col gap-4">
        ...
        <div className="text-small text-default-600">API used: AFS list, write, move, and read. This does not register a dataset in openBIS.</div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <Button variant="flat" onPress={onDummyUpload} isLoading={dummyMutation.isPending}>
            Dummy upload
          </Button>
          <Button color="secondary" onPress={onUpload} isLoading={uploadMutation.isPending}>
            Upload to AFS
          </Button>
          {dummyMutation.isSuccess && <div className="text-small text-success">Dummy file written to {dummyMutation.data.afsPath}</div>}
        ...
        {dummyError && <div className="text-small text-danger">{dummyError.summary}</div>}
        {dummyError?.details && <pre className="whitespace-pre-wrap break-words text-xs text-default-600">{dummyError.details}</pre>}
