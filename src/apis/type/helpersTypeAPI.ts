...
export function convertObjectTypeDefinitionToOperations(
  ...
  const propertyTypeUpdates = convertPropertyTypesSchemaToUpdateOperations(
    schemaToUpdate ?? {},
  ...
  if (
    ...
    // If it does not exist, create a new SampleTypeCreation
    ...
    sampleTypeCreation.setMetaData({type: objectDefinition.code, baseType: objectDefinition.baseType? objectDefinition.baseType : ""});
  } else {
    // If it exists, prepare an update operation
