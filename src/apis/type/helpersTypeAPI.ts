...
import { getIsInitializing } from "../shared/environment";
import {
  PropertyTypesSchema,
 ...
 * Only updates settings during initialization mode to avoid overriding existing lab configurations.
...
export function convertObjectTypeDefinitionToOperations(
  ...
  if (
    ...
    const prefix = objectDefinition.generatedCodePrefix.length > 0
        ? objectDefinition.generatedCodePrefix
        : objectDefinition.code.slice(0, 4);
    sampleTypeCreation.setGeneratedCodePrefix(prefix.toUpperCase());
    ...
    const prefix = objectDefinition.generatedCodePrefix.length > 0
        ? objectDefinition.generatedCodePrefix
        : objectDefinition.code.slice(0, 4);
    sampleTypeUpdate.setGeneratedCodePrefix(prefix.toUpperCase());
  ...
  const updates = Object.entries(schema)
    .flatMap(([section, properties]) => {
      return properties.map((prop: any, index) => {
        ...
        // Note: dataType cannot be changed for existing property types in openBIS
        return update;
      });
    })
    .flatMap((it) => (it ? [it] : []));
...
export function convertObjectTypeDefinitionToUpdateOperations(
  ...
  // Only set description if it's not null/undefined and different from existing
  if (objectDefinition.description !== null && objectDefinition.description !== undefined) {
    update.setDescription(objectDefinition.description);
  }
  update.setAutoGeneratedCode(true);

  const newPrefix = objectDefinition.generatedCodePrefix.length > 0
    ? objectDefinition.generatedCodePrefix
    : objectDefinition.code.slice(0, 4);

  update.setGeneratedCodePrefix(newPrefix.toUpperCase());
